...
class MotifDS(Dataset):
    def __init__(self, images_root, motifs_root, train=True, bound_offset=5, image_size=128,
                 ...
                 blur=False,fontsize=50,bounding_style=False,bound_weight=3,broken=False):
    ...
    def __motif_style(self,motif):
        if self.__broken:
            vm_indices, vm_rows, vm_cols = get_image_indices(motif)
            motif = np.array(motif)
            size = vm_indices[0].size
            # index = np.random.choice(size,int(size*0.5))
            # alpha =  random.random()/2
            # #更改透明度 vm_indices[0][[1,8]]
            # motif[vm_indices[0][index], vm_indices[1][index], :3] = int(255*alpha)
            # 清除
            for i in range (2):
                index = np.random.choice(size, int(size * 0.8),replace=False)
                motif[vm_indices[0][index], vm_indices[1][index], ] = (0,0,0,0)
            # k_size = random.randint(1, 2)
            # motif = cv2.blur(motif, (1 + k_size * 2, 1 + k_size * 2))
            motif = Image.fromarray(motif)
        return motif
            ...
            while type(vm_rows) is int or len(vm_rows) == 0:
                ...
                if self.__vm_paths == 'shapes':
                    if random.random() < .5:
                        ...
                        motif = self.__motif_style(motif)
