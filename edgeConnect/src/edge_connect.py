    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint with edge model / joint model
                ...
                #调整大小，使匹配
                # eh, ew = edges.shape[2:4]
                # imgh, imgw = images.shape[2:4]
                #if eh!=imgh or ew !=imgw:
                    #images = np.array(Image.fromarray(images).resize([eh, ew]))
                    #需要批处理
                outputs = self.inpaint_model(images, edges, masks)
                outputs_merged = (outputs * masks) + (images * (1 - masks))
            ...
            if True or self.debug:
                ...
                _o =self.postprocess( outputs.detach())[0]
                imsave(_o, os.path.join(self.results_path, fname + '_outputs.' + fext))
                _o2 = self.postprocess(outputs_merged.detach())[0]
                imsave(_o2, os.path.join(self.results_path, fname + '_outputs_m.' + fext))
