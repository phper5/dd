...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_item(self, index):
        ...
        img = np.asarray(Image.open(self.data[index]).convert('RGB'))
        ...
        if imgh %4 != 0 :
            newh = imgh - imgh%4
        if imgw != neww or imgh != newh :
            img = self.resize(img, neww,newh,False)
    ...
    def load_mask(self, img, index):
        ...
        if mask_type == 6:
            ...
            if len(channels) == 4:
                return self.resize(channels[3], imgw,imgh, centerCrop=False)
